...
<nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
    <x-admin-links :href="route('admin.index')" :active="request()->routeIs('admin.index')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-shield-quarter mr-2 icon-colored'></i> Home
    </x-admin-links>

    <x-admin-links :href="route('admin.roles.index')" :active="request()->routeIs('admin.roles.index')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-shield-quarter mr-2 icon-colored'></i> Roles
    </x-admin-links>

    <x-admin-links :href="route('admin.permissions.index')" :active="request()->routeIs('admin.permissions.index')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-lock-alt mr-2 icon-colored'></i> Permissions
    </x-admin-links>

    <x-admin-links :href="route('admin.users.index')" :active="request()->routeIs('admin.users.index')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-group mr-2 icon-colored'></i> Users
    </x-admin-links>

    <x-admin-links :href="route('admin.users.create')" :active="request()->routeIs('admin.users.create')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-user-plus mr-2 icon-colored'></i> Add User
    </x-admin-links>

    <x-admin-links :href="route('matches.index')" :active="request()->routeIs('matches.index')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-football mr-2 icon-colored'></i> Add Matches
    </x-admin-links>

    <x-admin-links :href="route('admin.tickets.index')" :active="request()->routeIs('admin.tickets.index')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
    <i class='bx bx-ticket-alt mr-2 icon-colored'></i> Tickets
</x-admin-links>


    <x-admin-links :href="route('admin.transactions')" :active="request()->routeIs('admin.transactions')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        <i class='bx bx-credit-card mr-2 icon-colored'></i> Transactions
    </x-admin-links>

    <x-admin-links :href="route('admin.suggestions')" :active="request()->routeIs('admin.suggestions')" class="hover-link block px-4 py-2 mt-2 text-sm font-semibold text-gray-900">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg">
            <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-700">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-dropdown-link :href="route('logout')" class="dropdown-link block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg"
